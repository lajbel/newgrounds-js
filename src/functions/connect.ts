...
export function connect(appId: string, cipher: string, config?: NewgroundsConf) {
    ...
    ng.scoreboards = this.call('ScoreBoard.getBoards', 0, 0).result?.data?.scoreboards ?? [];
    ng.medals = this.call('Medal.getList', 0, 0).result?.data?.medals ?? [];
